...
Output: Three new Python modules (active_view_store.py, sse_broadcaster.py, migration), models.py cleaned of ActiveView, urls.py with SSE route registered, views.py with build_active_view_payload helper and api_active_view_stream endpoint, and a working `GET /api/active-view/stream/` endpoint that streams initial state.
</objective>
...
# Note: active_view.save() calls are Plan 02's concern — this check only covers Plan 01's read-path migration
grep -n "from terminal.models import ActiveView\|ActiveView.get_current()" terminal/views.py && echo "FAIL: ActiveView ORM references remain" || echo "PASS: No ActiveView ORM read-path references"

# 3. Django check passes (catches import errors)
python manage.py check

# 4. Message model still intact
python -c "from terminal.models import Message; print('Message model OK:', Message.__name__)"

# 5. SSE URL registered
python -c "
from django.urls import reverse
print('SSE route:', reverse('active_view_stream'))
"

# 6. get_active_view_json and api_active_view_stream use build_active_view_payload
grep -n "build_active_view_payload" terminal/views.py
```
  </verify>
  <done>Migration 0017 applied (ActiveView table dropped); `from terminal.models import ActiveView` and `ActiveView.get_current()` removed from views.py; `display_view_react` and `get_active_view_json` read from `get_state()`; `build_active_view_payload` exists and is called by both functions; `api_active_view_stream` added and registered at `/api/active-view/stream/`; `python manage.py check` passes; Message model still importable from terminal.models. Note: remaining `active_view.save()` calls in write endpoints are Plan 02's domain.</done>
</task>
...
# No ORM leaks in read-path functions (write endpoints handled by Plan 02)
...
- `build_active_view_payload(state)` helper exists in views.py with full multi-deck loading logic inlined (not stubbed)
