...
must_haves:
  truths:
    ...
    - "isGridEncounterMap branch added to EncounterMapDisplay to route new maps to new renderer"
...
14. **TokenLayer props** — TokenLayer receives `mapRooms`. Update the type annotation in the TokenLayer call to pass `mapData.rooms as any` temporarily — full TokenLayer update is in Plan 03. For now, pass `mapData.rooms` and TypeScript will complain; suppress with `// @ts-expect-error -- TokenLayer updated in 07-03`.
  </action>
...
Note: The multi-deck map branch also needs to check `isGridEncounterMap` for the current_deck. Add a similar check inside the `isMultiDeckMap` branch:
```typescript
if (mapData && isMultiDeckMap(mapData)) {
  ...
  // Use grid renderer if the deck is grid format
  const mapProps = {
...
The existing `isEncounterMap` branch remains for any old-format maps that may still exist in other data directories. Do not remove it.
  </action>
  <verify>cd /home/gjohnson/mothership/charon && npm run typecheck 2>&1 | grep -E "error TS|0 errors" | head -10</verify>
