...
4. **Bulk reveal/hide buttons** — per CONTEXT.md locked decision: both "reveal all" and "hide all" buttons must be available. Before adding anything, read the ROOM VISIBILITY section header in EncounterPanel.tsx to see what's already there.

   - If `handleRevealAll` / `handleHideAll` are already wired to visible buttons (EyeOutlined / EyeInvisibleOutlined icon buttons in the section header), the requirement is already met — do NOT add duplicate buttons. Skip this step.
   - If those functions exist but the buttons are absent from the JSX, **replace** the section header's action area entirely with explicit text-label buttons. Do not add new buttons alongside existing icon buttons:

   ```tsx
   {/* ROOM VISIBILITY section header actions — replace any existing icon-only buttons with these */}
   ...
   In either case, ensure no duplicate controls exist after the change.
