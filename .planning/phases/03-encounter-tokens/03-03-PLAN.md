...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/components/gm/MapPreview.tsx"
      ...
      contains: "tokens"
    - path: "src/utils/svgCoordinates.ts"
      provides: "SVG coordinate transform and grid snapping utilities"
      exports: ["screenToSVG", "snapToGrid", "getGridCell"]
  key_links:
    ...
    - from: "src/components/domain/encounter/TokenLayer.tsx"
      to: "src/utils/svgCoordinates.ts"
      via: "Import screenToSVG and snapToGrid for drag-to-move coordinate transforms"
      pattern: "import.*from.*svgCoordinates"
    - from: "src/components/gm/MapPreview.tsx"
      to: "src/utils/svgCoordinates.ts"
      via: "Import screenToSVG and snapToGrid for drop-to-place coordinate transforms"
      pattern: "import.*from.*svgCoordinates"
...
Output: TokenPalette, TokenImageGallery components in GM console, MapPreview extended as drop target, Token drag-to-move behavior, svgCoordinates utility, full GM encounter token workflow.
</objective>
...
<task type="auto">
  <name>Task 2: SVG coordinate utilities, MapPreview drop target, drag-to-move, and EncounterPanel integration</name>
  <files>
    src/utils/svgCoordinates.ts
    src/components/gm/MapPreview.tsx
    src/components/gm/EncounterPanel.tsx
  ...
  <action>
0. Create `src/utils/svgCoordinates.ts` with coordinate transform utilities:
   - `screenToSVG(svgElement: SVGSVGElement, screenX: number, screenY: number): { x: number; y: number }`:
     - Create a DOMPoint from screenX, screenY
     - Get the SVG element's screen CTM via `svgElement.getScreenCTM()`
     - If CTM is null, fall back to getBoundingClientRect-based calculation
     - Invert the CTM and apply to the point: `point.matrixTransform(ctm.inverse())`
     - Return the transformed `{ x, y }` in SVG coordinate space
     - This correctly handles SVG viewBox scaling, pan, and zoom
   - `snapToGrid(svgX: number, svgY: number, unitSize: number): { gridX: number; gridY: number }`:
     - Convert continuous SVG coordinates to discrete grid cell indices
     - `gridX = Math.floor(svgX / unitSize)`, `gridY = Math.floor(svgY / unitSize)`
     - Return `{ gridX, gridY }`
   - `getGridCell(svgElement: SVGSVGElement, screenX: number, screenY: number, unitSize: number): { gridX: number; gridY: number }`:
     - Convenience function combining screenToSVG + snapToGrid
     - Calls `screenToSVG(svgElement, screenX, screenY)` then `snapToGrid(result.x, result.y, unitSize)`
     - Return `{ gridX, gridY }`
...
1. Extend MapPreview.tsx to be a drop target for token placement:
   ...
   - Add a ref to the SVG element: `const svgRef = useRef<SVGSVGElement>(null)`
   ...
   - Add `onDrop` handler:
     ...
     - Import and use `getGridCell(svgRef.current, e.clientX, e.clientY, unitSize)` from `@/utils/svgCoordinates` to convert drop position to grid coordinates
     ...
     - **Revealed rooms only (per user decision):** Determine which room contains the target cell. Check room boundaries from mapData.rooms — find the room whose bounds (x, y, width, height) contain the target cell. If no room contains the cell, or if the room is not revealed (roomVisibility[roomId] === false), reject the placement.
...
2. Implement drag-to-move for existing tokens on the GM map:
   ...
   - In TokenLayer.tsx: implement the drag-to-move behavior:
     - Import `screenToSVG` and `snapToGrid` from `@/utils/svgCoordinates`
     - Get a ref to the parent SVG element (pass down from EncounterMapRenderer or use `closest('svg')`)
     - Track drag state: `isDragging`, `dragTokenId`, `dragPosition` (current mouse position in SVG coords)
     - On token drag start (mousedown): set `isDragging = true`, `dragTokenId = id`, call `e.stopPropagation()` to prevent map panning (per research anti-pattern #2)
     - On mouse move (attached to window when dragging): use `screenToSVG(svgEl, e.clientX, e.clientY)` to convert mouse position to SVG coords, then `snapToGrid(svgX, svgY, unitSize)` to calculate snapped grid position, show ghost token at snapped position
     ...
     - Only call API on mouseup (not during drag), so one API request per drag operation
...
5. Check GMConsole.tsx to ensure it passes the right props. The EncounterPanel already receives `activeView` and `onViewUpdate`. Make sure token state flows correctly: EncounterPanel manages local token state, makes API calls, and calls `onViewUpdate` so the player terminal picks up changes via polling.
  ...
  <done>
    svgCoordinates.ts exports screenToSVG, snapToGrid, and getGridCell. GM can drag tokens from palette onto map preview with grid snapping. Overlap prevention and revealed-room constraints enforced. GM can drag-to-move existing tokens. Inline popup status toggles and remove button call APIs. Clear all tokens works with confirmation. Token state syncs to player terminal via polling.
  </done>
</task>
